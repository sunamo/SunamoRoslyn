...
AspxCsToStandaloneAssembly(string from, string to, string baseClassCs, string nsBaseClassCs, List<string> skipAspx)
    {
        ...
        {
            string fnwoeAspxCs = Path.GetFileNameWithoutExtensions(fileAspxCs);
...
TF.ReadAllText(fileAspxCs);
                ...
                var ctorArgs = string.Join(KeyValueCollection(dict.OnlyAs(), onlyB, AllStringsSE.space, AllStringsSE.comma);
